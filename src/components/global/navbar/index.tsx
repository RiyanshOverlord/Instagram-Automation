"use client";
import { PAGES_BREAD_CRUMBS } from "@/constants/pages";
...
const Navbar = ({ slug }: Props) => {
  const { page } = usePaths();
  const currentPage = PAGES_BREAD_CRUMBS.includes(page) || page == slug;

  return (
    currentPage && (
      <div className="flex flex-col">
        <div className="flex gap-x-3 lg:gap-x-5 justify-end">
          <span className="lg:hidden flex item-center flex-1 gap-x-2">
