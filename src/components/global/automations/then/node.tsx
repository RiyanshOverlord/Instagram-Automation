...
import { Play, SendHorizonal, WandSparklesIcon } from "lucide-react";
  ...
  return !data?.data?.listener ? (
    ...
    <div className="w-full lg:w-10/12 relative xl:w-6/12 p-5 rounded-xl flex flex-col bg-[#1D1D1D] gap-y-3">
      ...
      <div className="flex gap-x-2">
        <WandSparklesIcon color="#4C84FF" />
        Then ...
      </div>
      <div className="bg-background-80 p-3 rounded-xl flex flex-col gap-y-2">
        <div className="flex gap-x-2 items-center">
          {data.data.listener.listener === "MESSAGE" ?( <SendHorizonal /> ): (<FaRobot />)}
          <p className="text-lg">
            {data.data.listener.listener === "MESSAGE" ? "Send the user a message" : "Let SmartAI handle the conversation" }
          </p>
        </div>
