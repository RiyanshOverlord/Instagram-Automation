...
const Keywords = ({ id }: Props) => {
  const { deleteMutation, keyword, onValueChange, onKeyPress } = useKeywords(id);
  const { latestVariable } = useMutationDataState(["add-keyword"]);
  const { data } = useQueryAutomation(id);

  // Note : Optimistic UI can be used here to show the newly added keyword immediately and delete the keyword immediately on user action.
